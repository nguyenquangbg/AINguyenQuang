...
export const ImageGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateImageFromPrompt(prompt, aspectRatio, referenceImage || undefined, useGeminiModel);
                  ...
                  <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                       ...
                       <div className="flex bg-slate-100 dark:bg-slate-700 rounded-lg p-1">
                           <button 
                            ...
                            >
                                Gemini Flash (Tốc độ cao)
                            </button>
                       </div>
                       <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 italic">
                           {useGeminiModel ? 'Gemini Flash tạo ra 4 biến thể nhanh chóng.' : 'Imagen 3 tạo ra hình ảnh chi tiết và nghệ thuật hơn.'}
                       </p>
                  </div>
              )}
